...
class QueryRepo:
    ...
    def refuse(
        ...
        *,
        ...
        nothing to store.
        """
        # C15 - `verification_unavailable` joins the post-generation family
        # rather than getting an `AnswerOutcome` of its own.
        #
        # `AnswerOutcome` is the family, not the reason: `answer_metrics.is_refusal`
        # matches on the `refused` prefix precisely so that refining the refusal
        # taxonomy does not move the metric (BR-127), and `refusal_reason` on the
        # same row is where the distinction now lives.
        #
        # `REFUSED_UNSUPPORTED` over the alternatives. `REFUSED_LOW_RELEVANCE`
        # would say retrieval was weak, and retrieval was fine - the answer had
        # been generated by the time this happened. `ERROR` would say the query
        # did not finish (`mark_error`), and it did: it returned a refusal page
        # with source links and committed its sentences, so counting it as a
        # crash swaps one false statement for another.
        #
        # That leaves the false-refusal contamination this reason was split out
        # to expose, and it is not fixed here. Run 609's msds-03 polluted the
        # rate because the *evaluation* scored an interrupted measurement as a
        # product outcome; `evaluation_service` now stores such an item as
        # `quota_exhausted`, which keeps it out of `done` and out of every rate.
        # Relabelling the outcome would have hidden the interruption instead of
        # excluding it.
        row.outcome = (
