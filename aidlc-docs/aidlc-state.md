...
- [x] **거부 정확도 개선 BR-73a (2026-09-02)** — "거부 정확도 개선 진행해줘".
      ...
      **거부 정확도 0.400 → 1.000 · 오거부 0.000 유지 · 검색 불변.**
      ...
      단위 676 / 통합 72
- [x] **검색전용 기준선 604 승격 + C8(b) 측정 착수 (2026-09-07)** — "둘다 진행".
      기준선이 처음으로 Recall@10(1.000)을 갖는다. 385 는 C4(b) 이전이라
      스냅샷·지표가 달라 계속 `incomparable` 이었다.
      C8(b) 허용오차 재교정은 노이즈 크기가 선행 조건이라, 온도 0 적용 후
      `--full` 반복 측정을 시작했다(한 번이 하루). 판정 기준은 검토서에 미리 기입
