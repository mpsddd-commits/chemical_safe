...
## 6. C7 검증 — 충실도가 회복됐는가 (2026-09-05 이후)

C7 수정(심판 근거 중복 제거)이 실제로 지표를 되돌렸는지 **새 `--full` 실행으로만**
확인할 수 있다. 기존 실행 387 은 옛 코드로 측정됐으므로 이어붙일 수 없다 —
스크립트가 지문 불일치로 거부한다(확인함).

```bash
scripts/resume_and_promote.sh new --no-promote
```

`--no-promote` 인 이유: 이 실행의 목적은 **가설 검증**이고, 승격은 결과를 보고
사람이 정한다(BR-129).
...
| 결과 | 뜻 |
|---|---|
| **1.000** | 진단이 맞았다. sub-07·sub-08 이 회복됐고, 원인은 근거 중복으로 인한 절단이었다 |
| 0.958 | 절반만 회복. 한 문항만 고쳐졌다는 뜻이니 남은 쪽을 다시 봐야 한다 |
| 0.917 유지 | **진단이 틀렸다.** 중복 제거로 근거는 들어갔는데 심판이 여전히 불충실이라면 원인이 다른 데 있다 |

문항 단위로 확인할 것:
```sql
select question_id, judge_faithful, judge_reason
from evaluation_item where run_id=<새 실행> and question_id in ('sub-07','sub-08');
```
`judge_reason` 이 여전히 "근거 본문에 없다"를 지목하면, 무엇을 지목하는지가
다음 단서다.

### 함께 볼 것 (분모가 달라졌다)
정답률·인용 정확도도 같이 움직일 수 있다. C7 은 심판이 보는 근거를 바꿨으므로
**정답률 판정에도 영향을 준다** — 충실도만 보지 말 것.

### 비용
심판 25콜(거부 5문항은 심판을 쓰지 않는다), 일일 한도 20 → **이틀 걸릴 수 있다.**
`partial` 로 멈추면 다음 날 `scripts/resume_and_promote.sh <id> --no-promote`.

---

## 5. 보고 — 무엇을 담아야 하는가

배치가 끝나면 아래를 보고한다. **완주 여부에 따라 담을 것이 다르다.**

### 완주했을 때

1. **커밋** — 개수와 각각의 한 줄 요지
2. **지표 변화** — 새 `full` 기준선의 값. 특히 **충실도 0.958 → ? 판정**:
   회복했으면 "law-09 진단이 맞았다", 아니면 "진단이 틀렸거나 다른 원인이
   있다"를 근거와 함께. 정답률·인용은 BR-73a 로 분모가 달라져 **원인 분해가
   불가능**하다는 단서를 반드시 붙일 것
3. **백로그 정리 결과** — 요약이 아니라 내용:
   - 이번에 완료 처리된 항목: 개수와 목록
   - 남은 항목: 개수와 그룹별 분포
   - **순위가 바뀐 항목과 그 이유** — 보안 기본이 끝나면 순서가 달라진다
   - **작업하다 새로 발견해 추가한 항목** — 대개 생긴다. 없으면 없다고 적을 것
   - 남은 커밋 수 → 며칠치인지
4. **관리자 계정 만드는 명령** — B3 을 했으면 이것이 없으면 관리 화면에 못
   들어간다. 가입 URL 과 `grant-admin` 명령을 그대로 적을 것
5. **D5 결과** — 지웠는지, 보존한 로그 경로

### 완주하지 못했을 때

1. **어디서 멈췄는가** — 채점 수, 상태, 원인(할당량인지 다른 것인지)
2. **무엇을 하지 않았는가** — 2·3·4단계는 승격이 선행조건이라 시작하지 않는다.
   그것이 판단이지 누락이 아님을 분명히 할 것
3. **다음에 무엇을 하면 되는가** — 명령 그대로
4. 그날 남긴 커밋 (있으면)

**공통**: 지표가 안 움직인 것도 적는다. 하루 커밋 수가 10에 못 미쳤으면
그 이유를 적는다 — 숫자를 채우지 않은 것은 실패가 아니라 원칙이다.
